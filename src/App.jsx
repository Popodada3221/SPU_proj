...
import { calculateNetworkParameters, validateNetwork } from './utils/calculations';
...
function App() {
  ...
  const [isGostModalOpen, setGostModalOpen] = useState(false);
   ...
   const liveResults = calculationResults 
            ? {
                ...calculationResults,
                tasks: calculationResults.tasks.map(task => {
                  const override = userOverrides[task.id];
 ...
 const handleExportToWord = async () => {
  ...
  try {
    ...
    await generateAndSaveWordReport({
      ...
      networkRenderMode: networkDiagramRef.current?.getCurrentRenderMode() || 'aoa'
    });
...
const handleTaskUpdate = async (taskId, updates) => {
  ...
  const newOverrides = {
    ...userOverrides,
    [taskId]: { ...(userOverrides[taskId] || {}), ...updates },
  };
  ...
  await new Promise(resolve => setTimeout(resolve, 0));
   ...
   const recalculatedData = calculateNetworkParameters(project.tasks, newOverrides);
  
   setCalculationResults(prev => ({
    ...recalculatedData,
    tasks: prev.tasks.map(originalTask => {
      const updatedTask = recalculatedData.tasks.find(t => t.id === originalTask.id);
      return updatedTask ? { ...originalTask, ...updatedTask } : originalTask;
    })
  }));
 ...
 const handleLoadHelp = () => {
  ...
  return (
    <div className={`min-h-screen w-full bg-background text-foreground ${isNetworkFullScreen ? 'fullscreen-network' : ''}`}>
      {isNetworkFullScreen && calculationResults && (
        <div className="fixed inset-0 z-50 bg-background">
          <NetworkDiagram
            ref={networkDiagramRef}
            results={liveResults}
            onRenderModeChange={setNetworkRenderMode}
            isFullScreen={true}
            onToggleFullScreen={() => setIsNetworkFullScreen(false)}
          />
        </div>
      )}
      {!isNetworkFullScreen && (
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 app-container" style={{ transform: `scale(${appZoom})` }}>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          ...
          <TabsContent value="results" className="space-y-4">
            {calculationResults ? (
              <CalculationResults results={liveResults} project={project} />
            ) : (
              <Card>
            ...
            <TabsContent value="network" className="space-y-4">
              {calculationResults ? (
                <NetworkDiagram
                  ...
                  results={liveResults}
                  onRenderModeChange={setNetworkRenderMode}
                  isFullScreen={isNetworkFullScreen}
                  onToggleFullScreen={() => setIsNetworkFullScreen(!isNetworkFullScreen)}
                />
              ) : (
              <Card>
              <CardContent className="text-center py-8">
          ...
          <TabsContent value="analysis" className="space-y-4">
            <Tabs defaultValue="dashboard" className="space-y-4">
              ...
              <TabsContent value="dashboard">
                {calculationResults ? (
                  <ProjectDashboard results={liveResults} project={project} />
                ) : (
                  <Card>
                    <CardContent className="text-center py-8">
                      <BarChart3 className="h-12 w-12 mx-auto text-gray-400 mb-4" />
              ...
              <TabsContent value="resources">
                {calculationResults ? (
                  <ResourceManagement results={liveResults} project={project} />
                ) : (
                  <Card>
                    <CardContent className="text-center py-8">
            ...
            <Tabs defaultValue="export" className="space-y-4">
              ...
              <TabsContent value="export">
                ...
                <EnhancedExport 
                    ...
                    networkRenderMode={networkRenderMode}
                  />
              </TabsContent>
	  ...
	  <div style={{ position: 'absolute', left: '-9999px', top: '-9999px', zIndex: -1 }}>
	    <NetworkDiagram ref={networkDiagramRef} results={liveResults} onRenderModeChange={() => {}} />
	    <GanttChart ref={ganttChartRef} results={liveResults} project={project} />
	  </div>
)}
    </div>
  );
