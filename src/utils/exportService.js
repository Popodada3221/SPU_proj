...
export const generateAndSaveWordReport = async ({ baseline, optimized, networkRenderMode = 'aoa' }) => {
  try {
    ...
    const networkModeText = networkRenderMode.toUpperCase() === 'AON' ? ' (AON - работа-узел)' : ' (AOA - работа-ребро)';
    await addImageAsNewSection(optimized.networkImage, "Приложение А. Сетевой график" + networkModeText, true); 
